...
io.on('connection', (socket) => {
    ...
    socket.on("my-role", function(data){
        if(data.role == "frog"){
            ...
            //check if there's a conductor online
            //here we can't use socket.emit because we are sending it specifically to the conductor, instead of a random socket
            if(conductor){
    ...
    socket.on("disconnect", function(){
        ...
        if(idx > -1){
            ...
            //if the conductor disconnects
            conductor = undefined;
        }

        //if a conductor is online
        if(conductor){
